  ...
  setup() {
    ...
    const enviarEdicion = async () => {
      if (obraAEditar.value) {
        try {
          const respuesta = await fetch(`/api/obras/${obraAEditar.value.obraID}`, {
            method: 'PUT',
            headers: {
          ...
          mostrandoFormularioEdicion.value = false; // Oculta el formulario después de la actualización
          await cargarObras(); // Recarga la lista de obras para reflejar los cambios
        } catch (error) {
      ...
      <div v-if="seccionActiva === 'obras' && mostrandoFormularioEdicion && obraAEditar">
        <form @submit.prevent="enviarEdicion">
          ...
          <div v-for="(actor, index) in obraAEditar.actores" :key="actor.actorId">
            <input type="text" v-model="actor.nombre" placeholder="Nombre del actor" />
